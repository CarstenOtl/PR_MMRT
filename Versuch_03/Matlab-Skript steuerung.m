...
% Länge der Kette in m
L = 62*0.019; 
% Dichte pro Länge (kg/m)
rho = 2.3;
% Masse des Schlittens mit allen starr verbundenen Teilen (kg)
r = 0.150/2/pi; %Radius des Riementriebs
i = 3; %Übersetzung: Drehzahl Motor zu Drehzahl Riementrieb
M = (0.94 + 0.45) + 25.01e-5 * i^2 /r^2 + 0.09*2 + 2*1.8e-4 / r^2;  %in kg (Masse_Schlitten mit Aufbau) + Trägheit Anker projeziert auf Bewegung Schlitten
% + Gewicht Riemen + Trägheit der Scheiben
% Erdbeschleunigung
g = 9.81; 
% Zeitkonstante T 
T = 2*sqrt(L/g);
% Zeiten für die Bewegung
t_ende = 2.75*T;
t_1 = T;
t_2 = t_ende - T;
% Koeffizienten für die analytische Bestimmung von gamma
a_5 = 0.2;
a_4 = -0.5*(t_1+t_2);
...
% Berechnung der Wunschtrajektorie für das Kettenende 
t=0:0.005:t_ende;
...
% in der Funktion int_wagen_pos wurde der Integrand für das zu
% lösende Integral definiert
for index=1:size(t,2)
...
%% Hausaufgabe 
% Bestimmen Sie ähnlich wie die Wagenposition w(0,t) die zeitlichen und
% örtlichen Ableitungen: 
% dw(0,t)/dt = Geschwindigkeit Wagen
% d^2w(0,t)/dt^2 = Beschleunigung Wagen
% phi = dw(0,t)/dx = Kettenwinkel.
% Sie müssen hierzu nur die Funktionen int_wagen_v,int_wagen_a und
% int_wagen_phi vervollständigen (nicht das steuerung.m Skript).
...
% Kraft auf Wagen durch Kette und Trägheitskrraft des Wagens
F_m =  a_wagen * M;
...
% Summe aus Trägheitskraft + Reibung 
F = F_m+F_r;
% Berechnung der dafür nötigen Spannung unter berücksichtigung der induzierten
% Spannung
...
%% Rückfahrt der Kette durch Spiegelung der Trajektorien
T_pause = 4; % Pause am Zielpunkt
% Zeit für die Initialisierung des Systems
Zeitinit=10;
% Aufbau eines Zeitvektors für Hin- und Rückfahrt
t_gesamt        = [t,t(end)+0.001,t+t(end)+T_pause,2*t(end)+T_pause+0.01,2*t(end)+2*T_pause];
% Umkehrung der Werte für die Rückfahrt
...
% Aufbau der Vektoren für 5 malige Wiederholung des Vorgangs
